...
def train(nb_epoch, batch_size, store_name, resume=False, start_epoch=0, model_path=None):
    ...
    trainset = torchvision.datasets.ImageFolder(root=r"E:\testdata\dataset\tun_class\train", transform=transform_train)
    ...
    for epoch in range(start_epoch, nb_epoch):
        ...
        if epoch < 5 or epoch >= 80:
            val_acc, val_acc_com, val_loss = test(net, CELoss, 4)
...
train(nb_epoch=200,             # number of epoch
         ...
         store_name='fish',     # folder for output
